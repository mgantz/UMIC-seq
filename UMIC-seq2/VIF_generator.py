...
parser = argparse.ArgumentParser(description="""V1 - VIF_generator: Generates variant identity file (VIF) from polished Nanopore amplicon sequences""")
...
#set up probe seq and sequence seq variables - .seq command gives sequence SeqIO reads more info than that
#.fasta file has to be a good file, made by snapgene or so. There is more info in the file that it appears when opening it in text edit. 
...
#function slices sequence string between UMI_start and UMI_end and returns UMI as string 
...
for c in clusters:
    ...
    #very few reads are not readable by SeqIO -> this exception handels this (is counted by NoSeqIOread, should be very seldom: once in IRED dataset)
    try:
